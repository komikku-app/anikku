...
public class JSONObject {
     ...
     *
     * Some prebuilt anime extensions (e.g. Miruro) are compiled against a
     * Kotlin build of org.json and reference {@code JSONObject.NULL} as
     * {@code JSONObject$Companion.getNULL()}:
     *
     * <pre>getstatic  org/json/JSONObject.Companion
     * invokevirtual org/json/JSONObject$Companion.getNULL()</pre>
     *
     * The stock JSON-java artifact has no companion, which made those
     * extensions throw {@code NoSuchFieldError: Companion} at runtime. This
     * static field (plus the {@code JSONObject$Companion} class in this
     * package) satisfies that bytecode; {@code getNULL()} returns the real
     * {@link #NULL} sentinel, so behavior is identical to {@code NULL}.
     */
